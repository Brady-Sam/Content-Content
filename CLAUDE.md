...
- **The footer "Content counter" is derived, not written.** A `pre-commit` hook in `site/.git/hooks/` recounts the word "content" in the visible body copy of every published page (brand name included, so "Content Content" is two; `case-studies/knowledge-hub` excluded) and writes `content-count.json`, staging it into the commit. Each footer carries a baked-in fallback number and a script that swaps in the JSON value on load. The counter's own markup and script are wrapped in `<!-- content-counter -->` comments and stripped before counting, so it never counts itself. **Never hand-edit the number or `content-count.json`.** Same as `pre-push`: the hook swallows every error and exits 0, git does not version it, so after a fresh clone put it back from the copy kept at `../internal-docs/pre-commit`. The count logic (an inline Python block) lives entirely in the hook, there is no separate script. The counter copy is documented in `drafts/Homepage.md` with the rest of the footer.
